---
id: event.ftgo.rabbitmq.user.address.add-address
kind: Event
type: Event
title: user.address.add_address
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: user.address.add_address
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `user.address.add_address`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.address.add-address`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.address.add-address`
