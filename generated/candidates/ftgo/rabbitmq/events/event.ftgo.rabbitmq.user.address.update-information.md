---
id: event.ftgo.rabbitmq.user.address.update-information
kind: Event
type: Event
title: user.address.update_information
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: user.address.update_information
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
# user.address.update_information
...
- Broker identity: `user.address.update_information`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.address.update-information`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.address.update-information`
