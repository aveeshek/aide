---
id: event.ftgo.rabbitmq.user.profile.resend-auth-code
kind: Event
type: Event
title: user.profile.resend_auth_code
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: user.profile.resend_auth_code
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `user.profile.resend_auth_code`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.profile.resend-auth-code`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.profile.resend-auth-code`
