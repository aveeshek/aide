---
id: event.ftgo.rabbitmq.user.profile.create
kind: Event
type: Event
...
- Broker identity: `user.profile.create`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.profile.create`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.profile.create`
