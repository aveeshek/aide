---
id: event.ftgo.rabbitmq.user.profile.delete-account
kind: Event
type: Event
title: user.profile.delete_account
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: user.profile.delete_account
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `user.profile.delete_account`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.profile.delete-account`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.profile.delete-account`
