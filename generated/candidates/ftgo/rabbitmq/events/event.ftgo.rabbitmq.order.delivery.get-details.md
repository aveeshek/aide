---
id: event.ftgo.rabbitmq.order.delivery.get-details
kind: Event
type: Event
title: order.delivery.get_details
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: order.delivery.get_details
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `order.delivery.get_details`
- Correlation: `consumer_only`
- Mechanism: `rpc`
...
- `service.ftgo.order` `CONSUMES` -> `event.ftgo.rabbitmq.order.delivery.get-details`
