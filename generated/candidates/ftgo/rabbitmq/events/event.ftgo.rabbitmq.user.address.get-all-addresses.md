---
id: event.ftgo.rabbitmq.user.address.get-all-addresses
kind: Event
type: Event
title: user.address.get_all_addresses
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: user.address.get_all_addresses
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
# user.address.get_all_addresses
...
- Broker identity: `user.address.get_all_addresses`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.address.get-all-addresses`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.address.get-all-addresses`
