---
id: event.ftgo.rabbitmq.order.delivery.update-status
kind: Event
type: Event
title: order.delivery.update_status
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: order.delivery.update_status
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `order.delivery.update_status`
- Correlation: `consumer_only`
- Mechanism: `rpc`
...
- `service.ftgo.order` `CONSUMES` -> `event.ftgo.rabbitmq.order.delivery.update-status`
