---
id: event.ftgo.rabbitmq.user.profile.update-profile
kind: Event
type: Event
title: user.profile.update_profile
status: candidate
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: user.profile.update_profile
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `user.profile.update_profile`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.profile.update-profile`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.profile.update-profile`
