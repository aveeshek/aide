---
id: step.ftgo.gateway.get.address.get-all-info.read.ftgo.user.customer-address
kind: FlowStep
type: FlowStep
...
- target: data_access.repository.db_repository.DatabaseRepository.fetch
  depth: 3
  hops:
  - caller: application.address.AddressService.get_all_addresses
    callee: domain.manager.UserManager.load
    call: UserManager.load
    repository: ftgo
    commit: 52b1fd1b5d808e32b7925e890f560445a8460e7a
    path: backend/microservices/user/src/application/address.py
    symbol: application.address.AddressService.get_all_addresses
    line_start: 57
    line_end: 57
    evidence_type: implemented
...
- type: CONTAINS
  ...
  evidence_type: implemented
- type: PRECEDES
  source: step.ftgo.gateway.get.address.get-all-info.consume.user.user.address.get-all-addresses
...
- `application.address.AddressService.get_all_addresses` -> `domain.manager.UserManager.load` (`backend/microservices/user/src/application/address.py:57`)
