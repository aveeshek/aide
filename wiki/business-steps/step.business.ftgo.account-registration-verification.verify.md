---
id: step.business.ftgo.account-registration-verification.verify
kind: FlowStep
type: FlowStep
title: 'Account registration and verification: POST /auth/verify'
status: approved
...
position: 2
user_flow: flow.ftgo.gateway.post.auth.verify
http_method: POST
path: /auth/verify
trigger: user_action
evidence_mechanism:
...
- type: CONTAINS
  ...
  evidence_type: implemented
- type: PRECEDES
  source: step.business.ftgo.account-registration-verification.register
  evidence_mechanism:
  - route_navigation
  - vuex_dependency
  repository: ftgo
...
# Account registration and verification: POST /auth/verify
...
- Layer: `business`
- Role: `user_flow_reference`
- Business flow: `business-flow.ftgo.ui.account-registration-verification` (position 2)
- References user flow: `flow.ftgo.gateway.post.auth.verify`
- Trigger: `user_action`
- Evidence mechanism: `route_navigation`, `vuex_dependency`
- Declared in: `ui/src/components/VerifyAccountPage.vue` (lines 75-75)
