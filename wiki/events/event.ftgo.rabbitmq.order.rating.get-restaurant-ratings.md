---
id: event.ftgo.rabbitmq.order.rating.get-restaurant-ratings
kind: Event
type: Event
title: order.rating.get_restaurant_ratings
status: approved
...
- type: PUBLISHES
  ...
  evidence_type: implemented
attributes:
  identity: order.rating.get_restaurant_ratings
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
# order.rating.get_restaurant_ratings
...
- Broker identity: `order.rating.get_restaurant_ratings`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.order.rating.get-restaurant-ratings`

## Consumers

- `service.ftgo.feedback` `CONSUMES` -> `event.ftgo.rabbitmq.order.rating.get-restaurant-ratings`
