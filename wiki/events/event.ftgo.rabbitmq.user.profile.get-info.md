---
id: event.ftgo.rabbitmq.user.profile.get-info
kind: Event
type: Event
title: user.profile.get_info
status: approved
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: user.profile.get_info
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `user.profile.get_info`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.profile.get-info`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.profile.get-info`
