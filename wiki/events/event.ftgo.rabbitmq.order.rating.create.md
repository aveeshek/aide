---
id: event.ftgo.rabbitmq.order.rating.create
kind: Event
type: Event
title: order.rating.create
status: approved
...
- Broker identity: `order.rating.create`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.order.rating.create`

## Consumers

- `service.ftgo.feedback` `CONSUMES` -> `event.ftgo.rabbitmq.order.rating.create`
