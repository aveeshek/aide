---
id: event.ftgo.rabbitmq.delivery.rating.update
kind: Event
type: Event
title: delivery.rating.update
status: approved
...
- type: PUBLISHES
  ...
  evidence_type: implemented
attributes:
  identity: delivery.rating.update
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `delivery.rating.update`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.delivery.rating.update`

## Consumers

- `service.ftgo.feedback` `CONSUMES` -> `event.ftgo.rabbitmq.delivery.rating.update`
