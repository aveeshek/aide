---
id: event.ftgo.rabbitmq.user.profile.logout
kind: Event
type: Event
...
- Broker identity: `user.profile.logout`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.user.profile.logout`

## Consumers

- `service.ftgo.user` `CONSUMES` -> `event.ftgo.rabbitmq.user.profile.logout`
