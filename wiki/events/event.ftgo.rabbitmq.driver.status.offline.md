---
id: event.ftgo.rabbitmq.driver.status.offline
kind: Event
type: Event
title: driver.status.offline
status: approved
...
- type: CONSUMES
  ...
  evidence_type: implemented
attributes:
  identity: driver.status.offline
  mechanism:
  - rpc
  broker_libraries:
  - rabbitmq_rpc
...
- Broker identity: `driver.status.offline`
- Correlation: `matched`
- Mechanism: `rpc`
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.driver.status.offline`

## Consumers

- `service.ftgo.location` `CONSUMES` -> `event.ftgo.rabbitmq.driver.status.offline`
