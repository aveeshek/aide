---
id: event.ftgo.rabbitmq.order.create
kind: Event
type: Event
title: order.create
status: approved
...
- `service.ftgo.gateway` `PUBLISHES` -> `event.ftgo.rabbitmq.order.create`

## Consumers

- `service.ftgo.order` `CONSUMES` -> `event.ftgo.rabbitmq.order.create`
