---
id: endpoint.ftgo.gateway.post.status.offline
kind: Endpoint
type: Endpoint
title: POST /status/offline
status: approved
...
attributes:
  method: POST
  effective_path: /status/offline
  decorator_path: /offline
  router_prefix: /status
  path_resolution: partial
  decorator: router.post("/offline", response_model=SuccessResponse)
  handler: application.routes.driver.online_status.change_status_offline
...
- Method: `POST`
- Effective path: `/status/offline`
- Path resolution: `partial`
- Handler: `application.routes.driver.online_status.change_status_offline`
- Declared in: `backend/gateway/src/application/routes/driver/online_status.py` (lines 42-48)
